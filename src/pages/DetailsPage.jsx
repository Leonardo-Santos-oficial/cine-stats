...
export default function DetailsPage() {
  ...
  return (
    <article
      style={{ display: 'grid', gap: '1.5rem', gridTemplateColumns: '1fr', marginTop: '1rem' }}
    >
      <Title text={`${data.title} • Cine-Stats`} />
      <div
      ...
      >
        ...
        <div>
          ...
          <div className="meta" style={{ marginTop: '.75rem' }}>
            <span className="badge" title="Data de lançamento">
              {data.release_date}
            </span>
            <span className="badge" title="Nota média">
              ★ {data.vote_average?.toFixed(1)} ({data.vote_count})
            </span>
            <span className="badge" title="Gêneros">
              {genres}
            </span>
          </div>
        </div>
      ...
      >
        <h2 id="chart-title" style={{ marginTop: 0 }}>
          Análise rápida
        </h2>
